...
import { copy, readdir, remove, rename, stat, unlink, ensureFile } from 'fs-extra'
...
async function buildElectron(config: Configuration, dir: boolean) {
  ...
  if (existsSync('build/output/mac/Gerda Minecraft Launcher.app/Contents/Resources/app.asar')) {
    await writeHash('sha256', 'build/output/mac/Gerda Minecraft Launcher.app/Contents/Resources/app.asar', 'build/output/mac/GErda Minecraft Launcher.app/Contents/Resources/app.asar.sha256')
  }
...
async function start() {
  ...
  await copy(path.join(__dirname, '../gmcl-keystone-ui/dist'), path.join(__dirname, './dist/renderer'))
    ...
    await buildElectron({
      ...
      async artifactBuildCompleted(context) {
        if (context.target && context.target.name === 'appx') {
          await buildAppInstaller(version, path.join(__dirname, './build/output/gmcl.appinstaller'), electronBuilderConfig.appx!.publisher!)
        }
      },
    }, dir)
