import { ResourceDomain } from '@gmcl/runtime-api'
...
export async function tryPersistResource(resource: { fileName: string; domain: ResourceDomain; hash: string; path: string }, root: string, context: ResourceContext) {
  ...
  let existedEntry = await context.fileNameSnapshots[resource.domain].get(fileName).catch(() => undefined)
  while (existedEntry) {
    ...
    existedEntry = await context.fileNameSnapshots[resource.domain].get(fileName).catch(() => undefined)
  }
