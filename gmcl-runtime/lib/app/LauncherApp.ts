import { getPlatform } from '@gmcl/core'
import { InstalledAppManifest, Platform } from '@gmcl/runtime-api'
...
export class LauncherApp extends EventEmitter {
  ...
  readonly server: Server = createServer((req, res) => {
    this.protocol.handle({
      method: req.method,
      url: new URL(req.url ?? '/', 'gmcl://launcher'),
      headers: req.headers,
      body: req,
    }).then((resp) => {
  ...
  /**
   * Determine the root of the project. By default, it's %APPDATA%/gmcl
   */
  protected async setup() {
    ...
    // register gmcl protocol
    if (!this.host.isDefaultProtocolClient('gmcl')) {
      const result = this.host.setAsDefaultProtocolClient('gmcl')
      if (result) {
        this.log('Successfully register the gmcl protocol')
      } else {
        this.log('Fail to register the gmcl protocol')
      }
    }
  ...
  protected async getStartupUrl() {
    if (!IS_DEV && process.platform === 'win32') {
      ...
      if (process.argv.length > 1) {
        ...
        const protocolOption = process.argv.find(a => a.startsWith('gmcl://'))
        if (protocolOption) {
        ...
        this.log('Didn\'t find gmcl:// protocol')
      }
    }
