import { MinecraftLanDiscover } from '@gmcl/client'
import { ChecksumNotMatchError } from '@gmcl/installer'
import { InstanceManifest, PeerService as IPeerService, PeerServiceKey, PeerState, ShareInstanceOptions, UpnpMapOptions } from '@gmcl/runtime-api'
import { AbortableTask, BaseTask } from '@gmcl/task'
  ...
  constructor(@Inject(LauncherAppKey) app: LauncherApp,
    ...
    super(app, () => new PeerState(), async () => {
      ...
      const initNat = async () => {
        ...
        const existedMappings = mappings.filter(m => m.description.indexOf('GMCL Multiplayer') !== -1 && m.enabled)
        const findPorts = () => {
        ...
        if (existedMappings.length > 0) {
          ...
          for (const [priv, pub] of ports) {
            pendingMappings.push({
              description: `GMCL Multiplayer - udp - ${priv} - ${pub}`,
              protocol: 'udp',
              private: priv,
              public: pub,
              ttl: 24 * 60 * 60,
            }, {
              description: `GMCL Multiplayer - tcp - ${priv} - ${pub}`,
              protocol: 'tcp',
              private: priv,
              public: pub,
              ttl: 24 * 60 * 60,
    ...
    app.protocol.registerHandler('gmcl', ({ request, response }) => {
