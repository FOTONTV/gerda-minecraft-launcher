...
[![npm version](https://img.shields.io/npm/v/@gmcl/user.svg)](https://www.npmjs.com/package/@gmcl/user)
[![Downloads](https://img.shields.io/npm/dm/@gmcl/user.svg)](https://npmjs.com/@gmcl/user)
[![Install size](https://packagephobia.now.sh/badge?p=@gmcl/user)](https://packagephobia.now.sh/result?p=@gmcl/user)
[![Downloads](https://img.shields.io/npm/dm/@gmcl/user.svg)](https://npmjs.com/@gmcl/user)
[![Install size](https://packagephobia.now.sh/badge?p=@gmcl/user)](https://packagephobia.now.sh/result?p=@gmcl/user)
...
```ts
    import { login, offline, Authentication } from "@gmcl/user";
...
```ts
    import { validate, refresh } from "@gmcl/user";
...
```ts
    import { login, YggdrasilAuthAPI } from "@gmcl/user";
    const username: string;
    const password: string;
...
```ts
    import { lookupByName, GameProfile } from "@gmcl/user";
...
```ts
    import { lookup, GameProfile } from "@gmcl/user";
...
```ts
    import { lookup, GameProfile, getTextures } from "@gmcl/user";
    const userUUID: string;
    const gameProfile: GameProfile = await lookup(userUUID);
...
```ts
    import { lookup, GameProfile, setTexture } from "@gmcl/user";
    const userUUID: string;
...
```ts
    import { checkLocation, getChallenges, responseChallenges } from "@gmcl/user";
    const accessToken: string;
